...
public class BlockingConnectionHandler<T> implements Runnable, ConnectionHandler<T> {
    ...
    public BlockingConnectionHandler(Socket sock, MessageEncoderDecoderimpl reader, MessagingProtocolimp protocol,int connection_id,ConnectionImpl connection) {
        this.sock = sock;
        this.encdec = reader;
...
//                    }
    ...
    public synchronized void send(T msg) { //synchronized to prevent from two threads writing the same time to the same buffer
        ...
        try {
            out.write(((MessageOut)msg).encode());
            out.flush();
        } catch (IOException e) {
            e.printStackTrace();
        }
